...
************************************************************
*/
void Usart2_Init(unsigned int baud)      //8266
{
...
//...表示可变参数（多个可变参数组成一个列表，后面有专门的指针指向他），不限定个数和类型，                  
{
	...
	vsnprintf((char *)UsartPrintfBuf, sizeof(UsartPrintfBuf), fmt, ap);	//格式化,将参数fmt、ap指向的可变参数一起转换成格式化字符串，放string数组中，其作用同sprintf（），只是参数类型不同       
